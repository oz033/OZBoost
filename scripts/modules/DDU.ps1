...
<#
    Display Driver Uninstaller (DDU) auto-clean flow.
    Transcribed from Ultimate/5 Graphics/1 Driver Clean.ps1 (option 1: DDU Auto).

    Flow: download 7-Zip + DDU -> extract to %SystemRoot%\Temp ->
    configure DDU Settings.xml -> enable safe boot -> install RunOnce entry
    that runs DDU (clean + restart) in safe mode -> reboot.

    The embedded ddu.ps1 that runs in safe mode removes the safeboot flag
    and launches DDU with -CleanSoundBlaster -CleanRealtek -CleanAllGpus -Restart.
#>
...
& $WriteLog "[ddu] installing: 7-Zip File Manager"
...
& $WriteLog "[ddu] downloading: Display Driver Uninstaller"
...
& "$env:SystemDrive\Program Files\7-Zip\7z.exe" x "$env:SystemRoot\Temp\ddu.exe" -o"$env:SystemRoot\Temp\ddu" -y 2>&1 | ForEach-Object { & $WriteLog "$_" }
...
& $WriteLog "[ddu] restarting now to enter safe mode and run DDU"
