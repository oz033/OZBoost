...
<#
    Microsoft Edge & WebView2 removal / reinstall.
    Transcribed from Ultimate/6 Windows/22 Edge & WebView.ps1.

    mode='remove'   - original option 1: uninstall Edge + WebView2 fully
    mode='reinstall'- original option 2: download + install Edge & WebView2,
                      then apply Edge policies (uBlock force-install, HW accel
                      off, background mode off, startup boost off) and strip
                      the leftover logon/RunOnce/services/scheduled-task hooks
#>

param($PayloadArgs, $WriteLog)

$mode = $PayloadArgs.mode  # 'remove' | 'reinstall'
...
switch ($mode) {
    ...
    'remove' {
        ...
        & $WriteLog "[edge] remove: done"
        break
    }

    'reinstall' {

        & $WriteLog "[edge] reinstall: starting"
        ...
        & $WriteLog "[edge] reinstall: importing settings"
        ...
        & $WriteLog "[edge] reinstall: done"
        break
    }

    default {
        & $WriteLog "[edge] unknown mode: $mode"
    }
}
