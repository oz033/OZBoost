...
<#
    Graphics driver install + debloat + vendor settings.
    Transcribed from Ultimate/5 Graphics/3 Driver Install Debloat & Settings.ps1.

    The original presents a 1/2/3 NVIDIA/AMD/INTEL menu. We split by
    $PayloadArgs.vendor and run the chosen vendor path. Each path:
      - opens the vendor download page (interactive)
      - prompts the user to select the downloaded installer (OpenFileDialog)
      - extracts it with 7-Zip
      - debloats (NVIDIA: removes setup subfolders; AMD: edits XML/JSON;
        Intel: nothing)
      - installs the driver + control panel
      - sets vendor-specific registry settings
      - removes leftover vendor services (AMD/Intel sc deletes)

    Common tail for every vendor:
      - opens display / vendor panel / sound cpl for the user to configure
        resolution/refresh/primary display
      - disables AutoColorManagement for all monitors
      - enables MSI mode for all GPUs
      - reveals hidden taskbar icons
      - reboots
#>

param($PayloadArgs, $WriteLog, $RequestOpen)

$vendor = $PayloadArgs.vendor  # 'nvidia' | 'amd' | 'intel'

# Optional (NVIDIA): components the user chose to KEEP instead of debloating.
# @('physx', 'hdaudio') — omitted/empty = remove both (classic full debloat).
$keep = @($PayloadArgs.keep)

$ProgressPreference = 'SilentlyContinue'

# --- common: install 7-Zip -------------------------------------------------

& $WriteLog "[drv] installing: 7-Zip File Manager"
...
switch ($vendor) {
    ...
    'nvidia' {
        ...
        & $WriteLog "[drv] NVIDIA: opening driver download page"
        Start-Sleep -Seconds 5
        & $RequestOpen "https://www.nvidia.com/en-us/drivers"
        Pause
        Clear-Host

        & $WriteLog "[drv] NVIDIA: select downloaded driver"
        Start-Sleep -Seconds 5
        ...
        if ($keep -contains 'hdaudio') {
            & $WriteLog "[drv] NVIDIA: keeping HD-Audio (sound over HDMI/DP)"
        } else {
        ...
        if ($keep -contains 'physx') {
            & $WriteLog "[drv] NVIDIA: keeping PhysX (needed by some older games)"
        } else {
        ...
        & $WriteLog "[drv] NVIDIA: installing driver"
        ...
        & $WriteLog "[drv] NVIDIA: importing settings"
...
'@
    ...
    'amd' {
        ...
        & $WriteLog "[drv] AMD: opening driver download page"
        Start-Sleep -Seconds 5
        & $RequestOpen "https://www.amd.com/en/support/download/drivers.html"
        Pause
        Clear-Host

        & $WriteLog "[drv] AMD: select downloaded driver"
        Start-Sleep -Seconds 5
        Add-Type -AssemblyName System.Windows.Forms
        $Dialog = New-Object System.Windows.Forms.OpenFileDialog
        $Dialog.Filter = "All Files (*.*)|*.*"
        ...
        & $WriteLog "[drv] AMD: debloating driver"

        # extract driver with 7zip
        & "$env:SystemDrive\Program Files\7-Zip\7z.exe" x "$InstallFile" -o"$env:SystemRoot\Temp\amddriver" -y 2>&1 | ForEach-Object { & $WriteLog "$_" }
        ...
        & $WriteLog "[drv] AMD: installing driver"
        ...
        & $WriteLog "[drv] AMD: importing settings"
        & $WriteLog "[drv] AMD: IGNORE RSSERVCMD.EXE ERROR"
    ...
    'intel' {
        ...
        & $WriteLog "[drv] INTEL: opening driver download page"
        Start-Sleep -Seconds 5
        & $RequestOpen "https://www.intel.com/content/www/us/en/search.html#sortCriteria=%40lastmodifieddt%20descending&f-operatingsystem_en=Windows%2011%20Family*&f-downloadtype=Drivers&cf-tabfilter=Downloads&cf-downloadsppth=Graphics"
        Pause
        Clear-Host

        & $WriteLog "[drv] INTEL: select downloaded driver"
        Start-Sleep -Seconds 5
        Add-Type -AssemblyName System.Windows.Forms
        $Dialog = New-Object System.Windows.Forms.OpenFileDialog
        $Dialog.Filter = "All Files (*.*)|*.*"
        ...
        & $WriteLog "[drv] INTEL: debloating driver"

        # extract driver with 7zip
        & "$env:SystemDrive\Program Files\7-Zip\7z.exe" x "$InstallFile" -o"$env:SystemDrive\inteldriver" -y 2>&1 | ForEach-Object { & $WriteLog "$_" }

        & $WriteLog "[drv] INTEL: installing driver"
        ...
        & $WriteLog "[drv] INTEL: importing settings"
    ...
    default {
        & $WriteLog "[drv] unknown vendor: $vendor"
        return
    }
}

# --- common tail: panels, ACM off, MSI mode, taskbar, reboot ---------------

& $WriteLog "[drv] SET"
& $WriteLog "[drv] - SOUND"
& $WriteLog "[drv] - RESOLUTION"
& $WriteLog "[drv] - REFRESH RATE"
& $WriteLog "[drv] - PRIMARY DISPLAY"

# open display, nvidia & sound panels — via Request-Open (non-elevated)
try { & $RequestOpen "ms-settings:display" } catch { }
try { & $RequestOpen "shell:appsFolder\NVIDIACorp.NVIDIAControlPanel_56jybvy8sckqj!NVIDIACorp.NVIDIAControlPanel" } catch { }
& $RequestOpen "mmsys.cpl"
Pause
